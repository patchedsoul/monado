...
tracker3D_sphere_mono_instance_t* tracker3D_sphere_mono_create(tracker_instance_t* inst) {
	...
	if (i) {
		...
		i->params.blobColor=255;
		i->params.minArea=1;
		i->params.maxArea=1000;
		i->params.maxThreshold=255;
		i->params.minThreshold=250;
		i->params.thresholdStep=10;
		i->params.filterByConvexity=false;
		i->params.filterByInertia=false;
		i->params.filterByColor=false;
...
bool tracker3D_sphere_mono_queue(tracker_instance_t* inst,frame_t* frame) {
	...
	//threshold  our images (and invert with the '1' parameter passed to the threshold function, for blob tracker)
	//cv::threshold(internal->frame_gray,internal->frame_gray,100,255,CV_THRESH_BINARY_INV);
	...
	{
		//xrt_quat lastPos = lastTrackData.positions2D[i];
	...
	for (uint32_t i=0;i<internal->keypoints.size();i++)
	{
		...
		//intrinsics are 3x3
		float cx = internal->calibration.intrinsics[0];
		float cy = internal->calibration.intrinsics[4];
		float focalx=internal->calibration.intrinsics[2];
		float focaly=internal->calibration.intrinsics[5];

		float pixelConstant =0.1f;
		float z = blob.size * pixelConstant;
		float x = ((cx - blob.pt.x) * z) / focalx;
		float y = ((cy - blob.pt.y ) * z) / focaly;


		printf("d: %f sx: %f sy: %f x: %f y: %f z: %f %d\n",blob.size,blob.pt.x,blob.pt.y,x,y,z,internal->keypoints.size());
