                      ...
                      uint32_t mip_count)
{
     ...
     vmi.memoryTypeIndex = c->vk.device_memory_props getMemoryType(vmr.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
     ...
     VK_CHECK_RESULT(vkBindImageMemory(device, mappableImage, mappableMemory, 0));
        ...
        void comp_texture_destroy(comp_texture texture);
        {
...
static uint32_t get_memory_type(uint32_t typeBits, VkMemoryPropertyFlags properties, VkBool32 *memTypeFound = nullptr)
        {
