...
os_ble_notify_open(const char *mac, const char* endpoint,struct os_ble_device **out_ble)
{
    ...
    *out_ble = &bledev->base;
    ...
    //This is our GATT characteristic - the service and char endpoint numbers are device specific
    ...
    // create an empty array to pass as our options argument to AcquireNotify
    const char * container_signature = "{sv}"; //dbus type signature string
    DBusMessageIter iter, options;
    dbus_message_iter_init_append(msg, &iter); //attach it to our dbus message
    ...
    if (!dbus_connection_send_with_reply (bledev->conn, msg, &pending, -1)) { // -1 is default timeout
